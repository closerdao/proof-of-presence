...
import {PresenceToken} from '../typechain';
import {SignerWithAddress} from '@nomiclabs/hardhat-ethers/dist/src/signers';
import {DEFAULT_PRESENCE_TOKEN_DECAY_RATE_PER_DAY} from '../deploy/006_deploy_presenceToken';
...
const setup = deployments.createFixture(async () => {
  ...
  await presenceTokenContract.connect(deployer).setDecayRatePerDay(DEFAULT_PRESENCE_TOKEN_DECAY_RATE_PER_DAY);
...
describe('Token Decay Tests', function () {
  let presenceToken: PresenceToken, owner: SignerWithAddress, user: SignerWithAddress;
