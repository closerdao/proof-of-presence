...
import {DecayMathHarness, TestBase} from "./TestBase.sol";

contract DecayMathTest is TestBase {
    ...
    function test_PowerPreservesOddExponentFactor() public view {
        uint256 halfScale = SCALE / 2;

        assertEq(harness.powWithPrecision(halfScale, 3, SCALE), SCALE / 8);
        assertEq(harness.powWithPrecision(halfScale, 5, SCALE), SCALE / 32);
    }
