...
contract DAOAllowTransfersMock {
    ...
    function isTokenTransferPermitted(address from, address to, uint256 amount) external view returns (bool) {
        if (permittions[from][to] >= amount && permittions[from][to] > uint256(0)) return true;
        return false;
    }

    function addPermit(address from, address to, uint256 amount) public {
        permittions[from][to] = amount;
    }

    function doTransferFrom(address token, address from, address to, uint256 amount) public {
