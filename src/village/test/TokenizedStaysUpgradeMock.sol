...
contract TokenizedStaysUpgradeMock is TokenizedStays {
    /// @custom:storage-location erc7201:closer.storage.TokenizedStaysUpgradeMock
    struct UpgradeStorage {
        uint256 value;
    }

    function version() external pure returns (string memory) {
        return "tokenized-stays-upgrade-mock";
    }
    ...
    function _getUpgradeStorage() private pure returns (UpgradeStorage storage $) {
        uint256 storageLocation = erc7201("closer.storage.TokenizedStaysUpgradeMock");
