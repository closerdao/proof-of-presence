...
async function main(): Promise<void> {
  ...
  if (!args.manifest || !args.contract || !args.implementation || !args.version) {
    console.log(`Usage: npm run upgrade:v2:prepare -- --manifest <file> --contract <name>
  ...
  if (!UUPS_CONTRACTS.has(args.contract)) throw new Error(`'${args.contract}' is not a supported V2 UUPS contract`);
